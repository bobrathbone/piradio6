...
# Run the X-screen calibrator software
    ...
    if [[ ! -f ${CALIBRATION_CONF}.save ]]; then
        sudo cp ${CALIBRATION_CONF} ${CALIBRATION_CONF}.save
    fi
    sudo DISPLAY=:0.0 ${CALIBRATOR} --output-filename ${CALIBRATION_CONF}
    echo "New calibration details written to ${CALIBRATION_CONF}" | tee -a ${LOG}
